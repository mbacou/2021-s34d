...
options(
  ttt.theme="booktabs") 
st_options(
  plain.ascii=FALSE, bootstrap.css=FALSE, headings=FALSE,
...
ggBoxTest <- function(..., jitter=TRUE, grp.c=NULL, grp.s=NULL, cp=NULL, ref=NULL) {
  p = ggplot(...)
  if(jitter) p = p + geom_jitter(alpha=.5)
  p = p + 
    geom_boxplot(alpha=.7, outlier.shape=NA) +
    stat_summary(grp.c, fun.data=count, geom="text", size=3, color=1, vjust=.9)
  if(is.null(cp) && is.null(ref)) p = p +
      stat_compare_means(grp.s, label="p.signif", size=3.5, vjust=1, fontface="bold") +
      stat_compare_means(grp.s, label="p.format", size=3, vjust=3)
  else p = p +
      stat_compare_means(comparisons=cp, ref.group=ref, label="p.signif", size=3, vjust=0)
}
