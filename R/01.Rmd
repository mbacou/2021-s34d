...
hh_prod_cost[, type_alt := factor(type, labels=c(
  Seeds = "Seeds",
...
ggplot(
  hh[, .N, by=.(adm1_nm, group, age, ssd, gender)],
  aes(axis1=adm1_nm, axis2=gender, axis3=age, axis4=ssd, y=N)) +
  ...
  scale_x_discrete(limits=c("Province", "Gender", "Age", "Seed Distribution")) +
...
print(ctable(hh$group, hh$ssd, chisq=T, round.digits=0, 
  dnn=c("Group", "Seed System")),
  table.classes="table-condensed", method="render")
...
ggBoxTest(hh, aes(gender, costs_ha_ppp, fill=gender, color=gender)) +
...
ggBoxTest(hh, aes(group, costs_ha_ppp, fill=group)) +
  facet_wrap(~crop) +
  scale_y_continuous(labels=comma) +
  labs(x="", y="", fill="",
    title="Total Input Costs (PPP$ / ha) - Vietnam",
...
ggplot(hh_prod_cost[!type %in% c("Pesticides", "Seeds")], 
  ...
  scale_y_continuous(labels=percent) +
  stat_compare_means(label="p.signif", ref.group=".all.", size=3, fontface="bold") +  
  coord_flip() +
  facet_wrap(~crop) +  
  labs(x="", y="", fill="",
...
Farmers' gross profit margins by gender and across groups in both absolute terms and in relative terms as percentage of total input costs per hectare.
...
ggpairs(
  hh[, .(`seed club`=group, `age`=age_num, `years in group`=member_years,
