...
kbl(lbl, align="llccc", caption="Survey Codebook",)
...
ggplot(tbl, aes(gender, share, fill=type)) +
  ...
  facet_wrap(~ssd) +
  labs(y="", x="",
    title="Breakdown of Input Costs by Category",
...
ggplot(hh[!hhid %in% outlier], aes(costs_ha_ppp, yield_ha_kg)) +
  ...
  labs(x="", y="",
    title="Production Frontier (Output vs. Input)",
...
ggplot(hh, aes(gender, margin_ha_sh, fill=gender)) +
  ...
  scale_y_continuous(labels=percent) +
  labs(x="", y="", fill="",
    title="Gross Profit Margin in Relative Terms",
...
ggpairs(
  hh[, .(`seed club`=group, age, `costs`=costs_ha_ppp, `seed yield`=yield_ha_kg, `margin`=margin_ha_ppp)],
  upper = list(
    continuous="cor", 
    combo=wrap("summarise_by", color=pal)),
  lower = list(
    continuous=wrap("smooth", shape=NA), 
    combo=wrap("box_no_facet", fill=pal)),
  diag = list(
    continuous=wrap("densityDiag", fill=NA),
    discrete=wrap("barDiag", fill=pal)),
  title="Correlogram stratified by seed club"
)
...
ggpairs(
  hh[, .(gender, age, `costs`=costs_ha_ppp, `seed yield`=yield_ha_kg, `margin`=margin_ha_ppp)],
  upper = list(
    continuous="cor", 
    combo=wrap("summarise_by", color=pal[1:2])),
  lower = list(
    continuous=wrap("smooth", shape=NA), 
    combo=wrap("box_no_facet", fill=pal[1:2])),
  diag = list(
    continuous=wrap("densityDiag", fill=NA),
    discrete=wrap("barDiag", fill=pal[1:2])),
  title="Correlogram stratified by gender"
)
